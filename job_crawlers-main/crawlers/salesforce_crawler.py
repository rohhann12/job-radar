    ...
    try:
        ...
        for job_element in job_elements:
            try:
                ...
                print(f"Error parsing Salesforce job: {e}")
                continue

    except Exception as e:
        print(f"Salesforce crawler error: {e}")
    finally:
        driver.quit()
...
async def run_crawler_for_salesforce(receiverEmail=None):
    file_path = "urls/urls.json"
    ensure_company_document("Salesforce")

    with open(file_path, 'r') as file:
        position_urls = json.load(file)

    urls = position_urls.get("Salesforce", [])
    all_jobs = []
    num_jobs = 20
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for task in asyncio.as_completed(tasks):
            ...
            if jobs:
                ...
                print("New Salesforce jobs:", added_jobs)
                if added_jobs:
