    ...
    try:
        ...
        for job_element in job_elements:
            try:
                ...
                print(f"Error parsing Adobe job: {e}")
                continue

    except Exception as e:
        print(f"Adobe crawler error: {e}")
    finally:
        driver.quit()
...
async def run_crawler_for_adobe(receiverEmail=None):
    file_path = "urls/urls.json"
    ensure_company_document("Adobe")

    with open(file_path, 'r') as file:
        position_urls = json.load(file)

    urls = position_urls.get("Adobe", [])
    all_jobs = []
    num_jobs = 20
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for task in asyncio.as_completed(tasks):
            ...
            if jobs:
                ...
                print("New Adobe jobs:", added_jobs)
                if added_jobs:
